...
export async function generateMetadata({ params }) {
  ...
  if (genreSlugMap.has(processedSlug)) {
    ...
    return { title: `Vegamovies - ${genre.name} TV Series` };
  }

  if (CATEGORIES.includes(slug)) {
    const title = slug.replace(/_/g, ' ').toUpperCase();
    return { title: `Vegamovies - ${title} TV Series` };
  }
  ...
  return {
    title: `Vegamovies - ${tvShowData.name}`,
    description: tvShowData.overview,
    openGraph: {
      title: tvShowData.name,
      description: tvShowData.overview,
      url: `https://vegamovies-watch.netlify.app/tv-show/${slug}`,
      siteName: 'Vegamovies',
